...
public class TimePage extends BasePage {
    ...
    public TimePage filterByEmployee(String employeeName) {
        ...
        By option = By.xpath("//div[contains(@class,'oxd-autocomplete-option')]");
        WebElement employeeOption = wait.until(d -> {
            java.util.List<WebElement> options = d.findElements(option);
            if (options.isEmpty()) {
                return null;
            }
    ...
    public TimePage enterTimesheetRowData(String project, String activity, String hoursMon, String hoursTue, String hoursWed, String hoursThu) {
        ...
        WebElement projectField = wait.until(ExpectedConditions.elementToBeClickable(projectInput));
        projectField.clear();
        projectField.sendKeys(project);

        String sanitizedProject = project.replace("'", "\\'");
        String sanitizedActivity = activity.replace("'", "\\'");

        wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//div[contains(@class,'oxd-autocomplete-dropdown')]//*[contains(text(),'" + sanitizedProject + "')]"))).click();
        wait.until(ExpectedConditions.elementToBeClickable(activityDropdown)).click();
        wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//div[@role='listbox']//*[contains(text(),'" + sanitizedActivity + "')]"))).click();
